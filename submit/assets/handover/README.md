...
> ⚠ **루트 `TEAM_SYNC.md`(실서버 반영본)를 먼저 읽어라.** 이 묶음은 실서버 연동 *전* 작성분이라 노드명(node01~08 고정)·GPU 전제가 현재와 다르다. 실클러스터는 `master,node1~node13`(CPU, GPU 없음), 라우터 5개 등록 완료. 충돌 시 `TEAM_SYNC.md`가 우선.
...
- [ ] 노드 키 정합: 페이지가 노드명을 **하드코딩하지 않고 API대로 동적 렌더** (mock=node01~08 / 실데모=master,node1~13). 둘 다에서 정상이어야 함
