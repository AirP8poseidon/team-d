...
> ⚠ **먼저 루트 `TEAM_SYNC.md`를 읽어라.** 이 문서는 실서버 반영 *전* 작성분이다. 실클러스터는 `master,node1~node13`(GPU 없음)이며, **라우터 5개는 이미 동작·등록 완료**다(팀장에 등록 요청 불필요). 노드명·GPU·실행모드는 `TEAM_SYNC.md §2`가 우선한다.
