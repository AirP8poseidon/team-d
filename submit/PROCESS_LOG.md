...
### [#19] 위키 페이지 레이아웃 개선 — 1단 나열 → 2단(게시판 메인 + 서버환경 사이드 + 사용법 아코디언)
- 작성자(팀원): 김민주(kimminju)
- 목표: 세로로만 쭉 나열돼 핵심(노하우 게시판)이 맨 아래 묻히던 위키를, 사용빈도·정보위계에 맞게 구조적으로 재배치. 기능·데이터·API는 무변경, 시각/배치만.
- 에이전트에게 시킨 것(실제 프롬프트 핵심 인용):
  > "정보공유 탭에서 어떻게 배치를 하는게 좋을까? 구조적으로 보기 좋고 효율적으로"
  > "A로 한 번 수정해볼래?" → "오 이거 너무 좋다. 이 내용 내 브랜치에 빨리 푸쉬해줘"
- 사용한 기법(있으면): (b 도구연동 — 8100 격리 샌드박스 + Claude Preview MCP 라이브 검증(resize 1320·eval·screenshot)) / (a 구조 진단 후 옵션 A/B/C 제안 → 사용자 선택)
- 결과: ① **구조 진단** — 4섹션 1열 수직 나열의 문제 3가지(매일 쓰는 게시판이 맨 아래·사용빈도↔위치 불일치·참조vs상호작용 위계 평평) 도출, A(2단)/B(접기)/C(압축) 제안. ② **A안 구현(샌드박스 먼저)** — `wiki-grid`(좌 1.85fr 게시판=메인 / 우 1fr 서버환경=참조), 긴 '클러스터 사용 방법'(sbatch 코드블록)은 **하단 전체폭 `<details>` 아코디언**(기본 접힘)으로 분리해 세로 길이 단축+펼칠 때 코드 가독성 확보. `@media(max-width:960px)` 1단 폴백. **모든 fetch ID 보존**(serverList/searchInput/postList/writeForm/tagFilter/sortToggle)으로 기능 무변경. ③ **라이브 검증** — 1320px에서 그리드 663/359px, 글9·서버3 렌더, 아코디언 펼침 시 1044px 전체폭 코드 정상. ④ **브랜치 반영** — 샌드박스 vs 브랜치 `diff`로 A변경분만 차이임을 확인 후 파일 복사(byte 동일)→커밋·푸쉬.
- 막힘 → 해결: A안 원안은 '사용법을 우측 사이드 아코디언'이었으나 359px 사이드에선 sbatch 코드가 가로 스크롤로 읽기 불편 → **하단 전체폭 아코디언**으로 조정(접힘으로 세로 절약 + 펼침 시 가독성 둘 다 확보).
