...
### [#26] 실행 스크립트 세트 — Run/ 디렉토리(더블클릭 실행 + 사용법)
- 작성자(팀원): LeeSeongHoo (팀장)
- 목표: 누구나 더블클릭으로 서버를 띄우도록 실행 파일(.bat/.sh)과 사용법을 별도 `Run/`에 정리.
- 에이전트에게 시킨 것(실제 프롬프트 핵심 인용):
  > "서버 실행하는 쉘이나 bat 실행파일 만들어주고 사용법과 함께 별도 Run 디렉토리로 저장해줘"
- 사용한 기법: (c) 재사용 산출물(운영 스크립트)
- 결과: `Run/` 생성 — `README.md`(사용법·외부공개·모드설명), `setup.bat/.sh`(venv+pip), `start_server.bat`(ingest 실데이터), `start_server_mock.bat`(mock·독립실행/채점용), `start_refresh.bat`(라이브 갱신 180s), `stop.bat`(서버+루프 종료), `start_server.sh`(mac/linux, 기본 mock·`COLLECTOR=ingest`로 실데이터). 경로는 `Run/`→`..\hpc-portal` 상대참조(검증: hpc-portal·.venv·main.py·refresh_loop.ps1 해석 OK). mock/ingest 두 모드로 분리해 클러스터 없는 환경에서도 즉시 시연 가능.
- 막힘 → 해결: 없음. (실행 검증은 기동 중인 데모 서버와 8000 충돌 방지를 위해 경로 해석 검증으로 대체 — 명령 로직은 검증된 run_server.bat과 동일.)
