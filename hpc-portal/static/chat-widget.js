...
(function () {
  ...
  var css = ""
    ...
    + "background:linear-gradient(135deg, rgba(22,179,100,.92) 0%, rgba(14,95,56,.96) 100%);"
    + "-webkit-backdrop-filter:saturate(180%) blur(14px);backdrop-filter:saturate(180%) blur(14px);"
    + "color:#fff;border:1px solid rgba(255,255,255,.28);cursor:pointer;"
    + "box-shadow:0 10px 24px rgba(14,95,56,.30), inset 0 1px 0 rgba(255,255,255,.35);"
    + "font-size:26px;z-index:9998;transition:.15s;display:flex;align-items:center;justify-content:center;}"
    + ".cw-btn:hover{filter:brightness(1.06);transform:translateY(-1px);}"
    ...
    + "background:#fff;border:none;border-radius:20px;box-shadow:0 18px 48px rgba(31,42,55,.22);"
    + "z-index:9999;display:none;flex-direction:column;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,'Apple SD Gothic Neo','Malgun Gothic',Arial,sans-serif;}"
    + ".cw-panel.cw-show{display:flex;}"
    + ".cw-head{background:linear-gradient(135deg, rgba(22,179,100,.95) 0%, rgba(14,95,56,.97) 100%);"
    + "-webkit-backdrop-filter:saturate(180%) blur(16px);backdrop-filter:saturate(180%) blur(16px);"
    + "color:#fff;padding:13px 15px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,.18);}"
    + ".cw-head b{font-size:14px;color:#fff;}"
    + ".cw-head .cw-who{font-size:11px;color:rgba(255,255,255,.82);margin-left:6px;cursor:pointer;text-decoration:underline;}"
    + ".cw-x{background:none;border:none;color:rgba(255,255,255,.85);font-size:20px;cursor:pointer;line-height:1;}"
    + ".cw-x:hover{color:#fff;}"
    + ".cw-body{flex:1;overflow-y:auto;padding:12px;background:#f6f8f7;}"
    ...
    + "background:#fff;border:none;box-shadow:0 1px 3px rgba(31,42,55,.08);color:#1f2a37;}"
    + ".cw-row.cw-me .cw-bub{background:#16b364;color:#fff;box-shadow:0 2px 6px rgba(14,159,84,.25);}"
    + ".cw-name{font-size:11px;color:#8a94a3;margin:0 0 2px 2px;}"
    + ".cw-time{font-size:10px;color:#b6bdc7;margin-top:3px;text-align:right;}"
    + ".cw-row.cw-me .cw-time{color:#bfe9d2;}"
    + ".cw-foot{display:flex;gap:6px;padding:9px;border-top:1px solid #eef1f0;background:#fff;}"
    + ".cw-foot input{flex:1;border:1px solid #eef1f0;border-radius:12px;padding:9px 11px;font-size:13px;outline:none;}"
    + ".cw-foot input:focus{border-color:#16b364;box-shadow:0 0 0 3px rgba(22,179,100,.15);}"
    + ".cw-send{border:none;background:#16b364;color:#fff;border-radius:12px;padding:0 14px;font-size:13px;font-weight:650;cursor:pointer;}"
    + ".cw-send:hover{filter:brightness(1.05);}"
    // --- 헤더 검색 토글 ---
    + ".cw-head-acts{display:flex;align-items:center;gap:8px;}"
    + ".cw-ico{background:none;border:none;color:rgba(255,255,255,.85);font-size:16px;cursor:pointer;line-height:1;padding:0;}"
    + ".cw-ico:hover{color:#fff;}"
    + ".cw-search{display:none;padding:8px 10px;border-bottom:1px solid #eef1f0;background:#fff;}"
    + ".cw-search.cw-show{display:block;}"
    + ".cw-search input{width:100%;box-sizing:border-box;border:1px solid #eef1f0;border-radius:12px;padding:7px 10px;font-size:13px;outline:none;}"
    + ".cw-search input:focus{border-color:#16b364;box-shadow:0 0 0 3px rgba(22,179,100,.15);}"
    // --- 점유 선언 핀 섹션 ---
    + ".cw-pin{border-bottom:1px solid #eef1f0;background:#f4f6f5;padding:7px 10px;}"
    + ".cw-pin-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;}"
    + ".cw-pin-top b{font-size:11px;color:#1f2a37;letter-spacing:.3px;}"
    + ".cw-pin-add{border:1px solid #16b364;background:#fff;color:#0e9f54;border-radius:8px;padding:2px 8px;font-size:11px;font-weight:650;cursor:pointer;}"
    + ".cw-pin-add:hover{background:#16b364;color:#fff;}"
    + ".cw-pin-list{display:flex;flex-direction:column;gap:4px;}"
    + ".cw-pin-item{display:flex;align-items:center;justify-content:space-between;gap:6px;font-size:11.5px;color:#1f2a37;background:#fff;border:1px solid #eef1f0;border-radius:8px;padding:4px 7px;}"
    + ".cw-pin-item .cw-pin-txt{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}"
    + ".cw-pin-item .cw-pin-txt b{color:#0e9f54;}"
    + ".cw-pin-empty{font-size:11px;color:#8a94a3;padding:2px 0;}"
    + ".cw-pin-rel{flex:none;border:none;background:#eef1f0;color:#8a94a3;border-radius:7px;padding:2px 7px;font-size:10.5px;cursor:pointer;}"
    + ".cw-pin-rel:hover{background:#ef4444;color:#fff;}"
    + ".cw-pin-form{display:none;flex-direction:column;gap:4px;margin-top:5px;}"
    + ".cw-pin-form.cw-show{display:flex;}"
    + ".cw-pin-form input{border:1px solid #eef1f0;border-radius:8px;padding:5px 8px;font-size:12px;outline:none;}"
    + ".cw-pin-form input:focus{border-color:#16b364;box-shadow:0 0 0 3px rgba(22,179,100,.15);}"
    + ".cw-pin-form .cw-pin-submit{border:none;background:#16b364;color:#fff;border-radius:8px;padding:5px;font-size:12px;font-weight:650;cursor:pointer;}";
